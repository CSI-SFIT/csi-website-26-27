...
function generateTeamData(year) {
  ...
  DOMAINS.forEach((domain) => {
    ...
    data[domain.id] = members.map((member, i) =>
      enrichMember(member, domain.id, i, year),
    );
  });
  return data;
}
...
function SocialLinks({ socials }) {
  return (
    <div
      className={styles.memberSocials}
      role="list"
...
function DomainSection({ domain, members, year }) {
  ...
  const rolesToCheck = isMarketing2027
    ? marketingHeadRoles2027
    : headRoles[domain.id] || [];
  ...
  return (
    ...
    >
      <header className={styles.domainHeader}>
        <div
          className={`${styles.domainIcon} ${domain.iconBg} ${domain.iconColor}`}
        >
          <domain.icon size={22} aria-hidden="true" />
        </div>
        <h2 id={`${domain.id}-heading`} className={styles.domainTitle}>
          {domain.label}
        </h2>
      ...
      {heads.length > 0 && (
        <div
          className={`${styles.headsRow} ${isFaculty ? styles.facultyHeads : ""} ${isCore ? styles.coreHeads : ""}`}
          role="list"
        >
          {heads.map((member, i) => (
            <article
      ...
      {executives.length > 0 && (
        <div
          className={`${styles.executivesGrid} ${isCore ? styles.coreExecutives : ""}`}
          role="list"
        >
          {executives.map((member, i) => (
            <article
              ...
              className={styles.fadeIn}
              role="listitem"
              style={{ animationDelay: `${(i + heads.length) * 50}ms` }}
            >
...
export default function TeamPage() {
  ...
  return (
    <main
      className={`${styles.teamPage} ${inter.variable} ${lato.variable} font-lato`}
    >
